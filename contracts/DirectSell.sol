  ...
  IGasValueStructure
{
  ...
  constructor(
    ...
    if (
      ...
      ITokenRoot(paymentToken).deployWallet{ value: 0, flag: 128, callback: DirectSell.onTokenWallet }(
        address(this),
  ...
  function buildBuyPayload(uint32 callbackId, address buyer) external pure returns (TvmCell) {
    TvmBuilder builder;
  ...
  function onAcceptTokensTransfer(
    ...
    if (
      ...
      IDirectSellCallback(buyer).directSellSuccess{ value: Gas.FRONTENT_CALLBACK_VALUE, flag: 1, bounce: false }(
        callbackId,
        owner,
        buyer,
        nftAddress
      ...
      ITIP4_1NFT(nftAddress).transfer{ value: 0, flag: 128, bounce: false }(buyer, originalGasTo, callbacks);
      ...
      if (currentFee > 0) {
        ...
        ITokenWallet(tokenWallet).transfer{
          ...
          flag: 0,
          bounce: false
        }(currentFee, factoryDirectSell, Gas.FEE_DEPLOY_WALLET_GRAMS, originalGasTo, false, emptyPayload);
      }
    } else {
      if (endTime > 0 && now >= endTime && currentStatus == DirectSellStatus.Active) {
        ...
        ITIP4_1NFT(nftAddress).transfer{ value: 0, flag: 128, bounce: false }(owner, originalGasTo, callbacks);
      } else {
        IDirectSellCallback(buyer).directSellNotSuccess{ value: Gas.FRONTENT_CALLBACK_VALUE, flag: 1, bounce: false }(
          callbackId,
          nftAddress
        );
  ...
  function finishSell(address sendGasTo, uint32 callbackId) public {
    ...
    ITIP4_1NFT(nftAddress).transfer{ value: 0, flag: 128, bounce: false }(owner, sendGasTo, callbacks);
  }
  ...
  function closeSell(uint32 callbackId) external onlyOwner {
    ...
    require(msg.value >= calcValue(directSellGas.cancel), BaseErrors.not_enough_value);
    _reserve();
    IDirectSellCallback(owner).directSellClose{ value: Gas.FRONTENT_CALLBACK_VALUE, flag: 1, bounce: false }(
      callbackId,
      nftAddress
    );
    ...
    ITIP4_1NFT(nftAddress).transfer{ value: 0, flag: 128, bounce: false }(owner, owner, callbacks);
  }

  function upgrade(TvmCell newCode, uint32 newVersion, address sendGasTo) external override {
